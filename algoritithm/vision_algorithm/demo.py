...
def make_parser():
    ...
    parser.add_argument("-c", "--ckpt", default='./checkpoints/yolox_voc_s/best_ckpt.pth', type=str, help="ckpt for eval")
    parser.add_argument(
        "--device",
        default="cpu",
        type=str,
...
def get_model(exp):
    ...
    if getattr(exp, "model", None) is None:
        ...
        backbone = YOLOPAFPN(depth=0.33, width=0.5, in_channels=in_channels, act='silu')
        head = YOLOXHead(num_classes=2, width=0.5, in_channels=in_channels, act='silu')
    ...
    while True:
        ...
        if ret_val:
            ...
            if ch == ord("w") or ch == ord("W"):
                demo_result_save_dir = './demo_result'
                inx = os.listdir(demo_result_save_dir).__len__()
                res_img_path = os.path.join(demo_result_save_dir, f"{inx}.png")
                cv2.imwrite(res_img_path, result_frame)
                print(f'save img : {res_img_path}')
...
def main(exp, args):
    ...
    if args.demo == "result_image_show":
